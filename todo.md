...
### 6. منظومة حوارات التأكيد التفاعلية (Confirmation & Alert Dialogs)
- [x] بناء مكون `ConfirmationDialog.tsx` الموحد والمبني على Radix UI AlertDialog مع دعم التنبيهات التحذيرية والإعلامية.
- [x] تكامل نافذة مراجعة وتأكيد إرسال طلبات عروض الأسعار في `RFQForm.tsx`.
- [x] تفعيل حارس التراجع وحماية البيانات غير المحفوظة (`Unsaved Changes Guard`) في `RFQModal.tsx`.
- [x] ربط نافذة تأكيد تسجيل الخروج التفاعلية في `DashboardLayout.tsx`.

### 7. الصفحات القانونية والمحتوى المؤسسي والشفافية (Legal & Content Governance)
- [x] إنشاء صفحة الشروط والأحكام المؤسسية `TermsPage.tsx` وتوجيه المسار `/terms`.
- [x] إنشاء صفحة سياسة الخصوصية وأمان البيانات `PrivacyPage.tsx` وتوجيه المسار `/privacy`.
- [x] إنشاء صفحة الأسئلة الشائعة التفاعلية `FaqPage.tsx` وتوجيه المسار `/faq` مع دعم مكونات الأكورديون الميسرة.
- [x] تحديث سجل المسارات الشامل في `navigation.ts` والـ SEO Metadata في `seo.ts`.
- [x] إعادة هيكلة التذييل `Footer.tsx` إلى 4 أعمدة واضحة (العلامة، الاستكشاف، الحوكمة والشفافية، الاتصال المباشر).
- [x] توسيع حزمة الاختبارات لتأكيد سلامة المسارات الجديدة وتمرير كامل الفحوصات `pnpm check`, `pnpm test`, `pnpm build`.
